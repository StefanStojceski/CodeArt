...
function filterResources(resources, type) {
  // Filter by type.
  var filteredByType = resources.filter(item => item.type === type);
  // Get unique regions by filtered array with type.
  var uniqueSetByRegion = [...new Set(filteredByType.map(item => item.region))];
  // Empty object which will be filled later on.
  var finalObject = {};

  // Going through all unique region and adding the object with filtered values.
  uniqueSetByRegion.forEach(uniqueItem => {
      finalObject[uniqueItem] = filteredByType.filter(item => item.region === uniqueItem).map(singleItem => {
          var prefinalizedObject = {
              id: singleItem.id,
              name: singleItem.name,
              country: singleItem.code.split('_')[0].toUpperCase(),
              startDate: singleItem.startDate,
              endDate: singleItem.endDate
          };

          if( type === 'campaign' ) {
              return {...prefinalizedObject, campaign_id: singleItem.code.split('_')[1]}
          }

          if( type === 'order' ) {
              return {...prefinalizedObject, order_id: singleItem.code.split('_')[1]}
          }

          return prefinalizedObject;
      });
  });
...
console.log(filterResources(resources,"campaign"))
//console.log(filterResources(resources,"order"))